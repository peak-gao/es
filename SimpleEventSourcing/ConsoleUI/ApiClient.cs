...
namespace ConsoleUI
{
    public static class ApiClient
    {
        ...
        //public static async Task<IReadOnlyList<ScheduleDto>> GetAll(string enrolledIn, string numberOfCourses)
        //{
        //    var result = await SendRequest<List<ScheduleDto>>($"?enrolled={enrolledIn}&number={numberOfCourses}", HttpMethod.Get).ConfigureAwait(false);
        //    return result.Value;
        //}

        public static async Task<Result> AddSchedule(ScheduleDto dto)
        {
            Result result = await SendRequest<string>("/", HttpMethod.Post, dto).ConfigureAwait(false);
            return result;
        }

        public static async Task<Result> RemoveSchedule(Guid id)
        {
            Result result = await SendRequest<string>("/" + id, HttpMethod.Delete).ConfigureAwait(false);
            return result;
        }

        public static async Task<Result> UpdateSchedule(ScheduleDto dto)
        {
